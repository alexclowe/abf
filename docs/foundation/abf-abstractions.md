# ABF Abstractions Discovered from CiteRank

Patterns that emerged from designing CiteRank's 17-agent system that should be generalized into ABF framework features. These aren't CiteRank-specific — any business running on ABF would need them.
...
| # | Abstraction | Impact | Effort | Priority |
|---|------------|--------|--------|----------|
| 1 | **Communication Approval Queue** | Critical | Medium | P0 — Every ABF business sending external comms needs this |
| 2 | **Company Brain / Knowledge Base** | High | Low | P0 — Just directory convention + search integration |
| 3 | **Business Database** | High | Medium | P0 — Agents need structured data, not just memory |
| 4 | **Agent Roles / Archetypes** | High | Low | P1 — Template defaults, speeds up agent creation |
| 5 | **Cross-Agent Memory** | High | Medium | P1 — Team collaboration is broken without it |
| 6 | **Agent Inbox** | Medium | Medium | P1 — Better than current trigger hodgepodge |
| 7 | **Metrics Dashboard** | Medium | High | P2 — Nice but operators can use external tools |
| 8 | **Communication Router** | Medium | Low | P2 — Mostly wiring existing pieces together |
| 9 | **Workflow Templates** | Medium | Medium | P2 — Helpful but not blocking |
| 10 | **External Monitoring** | Low | High | P3 — Niche, could be MCP server |
